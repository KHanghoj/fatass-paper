...
(log, samples, pops, out) = sys.argv[1:]
d = np.load(log)
test = d.max(axis=2).argmin(axis=1)
cts = np.unique(test, return_counts=True)
labels = np.loadtxt(samples, dtype=str)
pops = np.loadtxt(pops, dtype=str)
...
for idx, (index, row) in enumerate(data.nlargest(10, "counts").iterrows()):
    ax.annotate(f"{row.samples}_{row.pops}_{row.counts}", (row.counts-2,40 + idx*5))
fig.savefig(f"{out}_worst_haplotype_per_window.png")
...
g.set_yscale("log")
g.set_title(f"W:{d_max_norm.shape[0]}, Haplotypes:{d_max_norm.shape[1]}, Nobs: {np.product(d_max_norm.shape)}")
fig.savefig(f"{out}_all_windows.png", bbox_inches='tight')
