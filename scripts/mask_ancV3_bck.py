    ...
    for x in range(N):
        ...
        for k in range(K):
            if not args.het_mask:
                mask1 = post[:, idx1, k]
                mask2 = post[:, idx2, k]
                mask_clusters1 =  np.logical_or(mask1<args.posterior_cutoff,
                                                mask2<args.posterior_cutoff)
                Ls[k][mask_clusters1, idx1, :] = missing
                Ls[k][mask_clusters1, idx2, :] = missing
            else:
                for idx in [idx1, idx2]:
                    mask = post[:, idx, k]
                    mask_clusters = mask < args.posterior_cutoff
                    Ls[k][mask_clusters, idx, :] = missing
                

        # if args.labels:
        #     newname1 = labels[x] + "_anc1"
        #     newname2 = labels[x] + "_anc2"
        #     labels[x] = newname1
        #     labels.append(newname2)

        # if args.names:
        #     names.append(names[x]+"_extra")


        # if not args.het_mask:
        #     ## one ancestry
        #     mask_clusters1 =  np.logical_or(mask1[:,0]<args.posterior_cutoff,
        #                                     mask2[:,0]<args.posterior_cutoff)
        #     L[mask_clusters1, idx1, :] = missing
        #     L[mask_clusters1, idx2, :] = missing

        #     ## other ancestry
        #     mask_clusters2 =  np.logical_or(mask1[:,1]<args.posterior_cutoff,
        #                                     mask2[:,1]<args.posterior_cutoff)
        #     L2[mask_clusters2, idx1, :] = missing
